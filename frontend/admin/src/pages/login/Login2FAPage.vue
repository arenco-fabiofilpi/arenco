<template>
  <div class="page-login q-pa-md flex flex-center">
    <q-card class="q-pa-md" style="max-width: 480px; width: 100%">
      <q-card-section class="text-center">
        <div class="text-h6">Verificação em duas etapas</div>
        <div class="text-subtitle2 q-mt-xs">Escolha um contato para receber o código</div>
      </q-card-section>

      <!-- Avisos -->
      <q-card-section v-if="alert.message">
        <q-banner
          :class="
            alert.type === 'error'
              ? 'bg-negative text-white'
              : alert.type === 'success'
                ? 'bg-positive text-white'
                : 'bg-grey-3'
          "
          rounded
          dense
        >
          <div class="row items-center no-wrap">
            <q-icon :name="alertIcon" class="q-mr-sm" />
            <div class="col">{{ alert.message }}</div>
          </div>
        </q-banner>
      </q-card-section>

      <!-- Seleção de contato + envio -->
      <q-card-section>
        <q-select
          v-model="selectedContactId"
          :options="contactOptions"
          option-value="id"
          option-label="label"
          emit-value
          map-options
          label="Contato"
          outlined
          dense
          :loading="loading.contacts"
          :disable="loading.contacts || loading.trigger || cooldownSeconds > 0"
          :hint="
            cooldownSeconds > 0
              ? `Aguarde ${cooldownSeconds}s para reenviar`
              : 'Selecione um contato e clique em Enviar código'
          "
          class="q-mb-md"
        >
          <template #prepend>
            <q-icon name="contacts" />
          </template>
          <template #no-option>
            <q-item>
              <q-item-section class="text-grey">
                {{ loading.contacts ? 'Carregando...' : 'Nenhum contato disponível' }}
              </q-item-section>
            </q-item>
          </template>
        </q-select>

        <q-btn
          label="Enviar código"
          color="primary"
          class="full-width q-mb-sm"
          :loading="loading.trigger"
          :disable="!selectedContactId || loading.contacts || cooldownSeconds > 0"
          @click="triggerOtp"
        />

        <div class="text-caption text-grey-7 q-mb-md" v-if="lastSentTo">
          Último envio para: <b>{{ lastSentTo }}</b>
          <span v-if="cooldownSeconds > 0"> — aguarde {{ cooldownSeconds }}s</span>
        </div>
      </q-card-section>

      <q-separator spaced />

      <!-- Input do OTP -->
      <q-card-section>
        <q-input
          v-model="otp"
          label="Código de verificação (OTP)"
          outlined
          dense
          maxlength="10"
          inputmode="numeric"
          :disable="loading.validate"
          :rules="[validaOtp]"
          @keyup.enter="validateOtp"
        >
          <template #prepend>
            <q-icon name="vpn_key" />
          </template>
        </q-input>

        <q-btn
          label="Validar código"
          color="secondary"
          class="full-width q-mt-md"
          :loading="loading.validate"
          :disable="!isOtpReady || loading.validate"
          @click="validateOtp"
        />
      </q-card-section>

      <!-- Rodapé -->
      <q-card-actions align="between">
        <q-btn flat icon="arrow_back" label="Voltar ao login" @click="logout" />
        <q-btn flat icon="help" label="Precisa de ajuda?" @click="showHelp = !showHelp" />
      </q-card-actions>

      <q-slide-transition>
        <div v-show="showHelp" class="q-pa-md text-caption text-grey-7">
          • Verifique caixa de entrada/SMS.<br />
          • Se não recebeu, aguarde o cooldown e tente novamente.<br />
          • Se o token expirar, peça um novo envio.
        </div>
      </q-slide-transition>
    </q-card>
  </div>
</template>

<script setup lang="ts">
  import { ref, computed, onMounted, onBeforeUnmount } from 'vue';
  import { useRouter } from 'vue-router';
  import { apiClientes } from 'src/boot/axios';
  import type { AxiosError } from 'axios';

  type ContactMethod = 'EMAIL' | 'CELLPHONE';

  interface UserContactDto {
    id: string;
    contactMethod: ContactMethod;
    contactValue: string;
  }

  interface TriggerResponse {
    result: 'SUCCESS' | 'FAIL';
    userContactDto: UserContactDto;
    message: string; // ex: "QUEUED"
  }

  interface ErrorPayload {
    code?: string; // "429 TOO_MANY_REQUESTS" | "401 UNAUTHORIZED" | "400 BAD_REQUEST" | ...
    message?: string; // mensagem amigável do back
    otpTokenValid?: boolean; // presente em 400 BAD_REQUEST
  }

  const router = useRouter();

  const loading = ref({
    contacts: false,
    trigger: false,
    validate: false,
  });

  const contacts = ref<UserContactDto[]>([]);
  const selectedContactId = ref<string | null>(null);
  const otp = ref('');
  const lastSentTo = ref<string | null>(null);

  const cooldownSeconds = ref(0);
  let cooldownTimer: number | null = null;

  const showHelp = ref(false);

  const alert = ref<
    { type: 'info' | 'success' | 'error'; message: string } | { type: null; message: '' }
  >({
    type: null,
    message: '',
  });

  const alertIcon = computed(() => {
    if (alert.value.type === 'error') return 'error';
    if (alert.value.type === 'success') return 'check_circle';
    return 'info';
  });

  const contactOptions = computed(() =>
    contacts.value.map((c) => ({
      ...c,
      label: formatContactLabel(c),
    }))
  );

  const isOtpReady = computed(() => otp.value.trim().length >= 4); // ajuste se seu OTP tiver tamanho fixo

  function setAlert(type: 'info' | 'success' | 'error', message: string) {
    alert.value = { type, message };
  }

  function clearAlert() {
    alert.value = { type: null, message: '' } as never;
  }

  function formatContactLabel(c: UserContactDto) {
    // exibe algo do tipo: "Celular • +55169XXXXX27" ou "E-mail • f*****i@..."
    const prefix = c.contactMethod === 'CELLPHONE' ? 'Celular' : 'E-mail';
    return `${prefix} • ${c.contactValue}`;
  }

  async function logout() {
    try {
      await apiClientes.post('/clientes-api/v1/auth/logout');
    } catch (error) {
      console.error('Erro ao fazer logout:', error);
    } finally {
      // Limpa os tokens e redireciona para login
      router.push('/login');
    }
  }

  async function loadContacts() {
    loading.value.contacts = true;
    clearAlert();
    try {
      const { data } = await apiClientes.get<UserContactDto[]>(
        '/clientes-api/v1/otp/contacts-available'
      );
      contacts.value = data;
      if (contacts.value.length === 0 || !contacts.value[0]) {
        setAlert('info', 'Nenhum contato disponível. Volte e faça login novamente.');
      } else if (!selectedContactId.value) {
        selectedContactId.value = contacts.value[0].id;
      }
    } catch (err) {
      const e = err as AxiosError<ErrorPayload>;
      setAlert('error', e.response?.data?.message || 'Falha ao carregar contatos.');
    } finally {
      loading.value.contacts = false;
    }
  }

  function startCooldown(seconds = 10) {
    cooldownSeconds.value = seconds;
    if (cooldownTimer) {
      window.clearInterval(cooldownTimer);
    }
    cooldownTimer = window.setInterval(() => {
      if (cooldownSeconds.value <= 1) {
        window.clearInterval(cooldownTimer!);
        cooldownTimer = null;
        cooldownSeconds.value = 0;
      } else {
        cooldownSeconds.value -= 1;
      }
    }, 1000);
  }

  async function triggerOtp() {
    if (!selectedContactId.value) return;
    loading.value.trigger = true;
    clearAlert();
    try {
      await apiClientes.post<TriggerResponse>(`/clientes-api/v1/otp/trigger`, {
        contactId: selectedContactId.value,
      });
      lastSentTo.value = contacts.value.find((c) => c.id === selectedContactId.value)
        ? formatContactLabel(contacts.value.find((c) => c.id === selectedContactId.value)!)
        : null;
      setAlert('success', 'Código enviado. Verifique sua caixa de entrada/SMS.');
      startCooldown(10); // conforme sua API retorna na mensagem de 429
    } catch (err) {
      const e = err as AxiosError<ErrorPayload>;
      const statusMsg = e.response?.data?.message || 'Não foi possível enviar o código.';
      if (e.response?.status === 429) {
        setAlert('error', statusMsg); // ex: "aguarde mais 10 segundos..."
        // inicia cooldown mínimo de segurança (se quiser extrair o número da mensagem, pode parsear aqui)
        startCooldown(10);
      } else {
        setAlert('error', statusMsg);
      }
    } finally {
      loading.value.trigger = false;
    }
  }

  const validaOtp = (val: string) => !!val?.trim() || 'Informe o código enviado';

  async function validateOtp() {
    if (!isOtpReady.value) return;
    loading.value.validate = true;
    clearAlert();
    try {
      await apiClientes.post('/clientes-api/v1/otp/authentication', {
        otp: otp.value.trim(),
      });
      setAlert('success', 'Verificação concluída. Redirecionando...');
      // pequeno delay para UX antes do push
      setTimeout(() => router.push('/admin/app'), 300);
    } catch (err) {
      const e = err as AxiosError<ErrorPayload>;
      const code = e.response?.data?.code || '';
      const msg = e.response?.data?.message || 'Falha na validação do código.';
      if (code.startsWith('401')) {
        // token inexistente/expirado/inválido -> peça novo envio
        setAlert('error', msg + ' Solicite um novo código.');
      } else if (code.startsWith('400')) {
        setAlert('error', msg); // exibe "Token Inválido. Restam 2 tentativas"
      } else {
        setAlert('error', msg);
      }
    } finally {
      loading.value.validate = false;
    }
  }

  onMounted(() => {
    loadContacts();
  });

  onBeforeUnmount(() => {
    if (cooldownTimer) {
      window.clearInterval(cooldownTimer);
      cooldownTimer = null;
    }
  });
</script>

<style scoped>
  .page-login {
    min-height: 80vh;
  }
  .full-width {
    width: 100%;
  }
</style>
