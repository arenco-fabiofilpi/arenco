<template>
  <div class="q-pa-md">
    <h1 class="q-mt-lg q-mb-md">E-mail's Enviados</h1>
    ...
    <q-table
      ...
      @request="fetchApi"
      no-data-label="Nenhum E-mail encontrado."
    />
  </div>
</template>
...
<script setup lang="ts">
  ...
  const columns = [
    { name: 'date', align: 'left' as const, label: 'Data de Criação', field: 'date' },
    { name: 'from', align: 'left' as const, label: 'Remetente', field: 'from' },
    { name: 'deliveredTo', align: 'left' as const, label: 'Destinatário', field: 'deliveredTo' },
    { name: 'subject', align: 'left' as const, label: 'Assunto', field: 'subject' },
    { name: 'messageId', align: 'left' as const, label: 'ID da Mensagem', field: 'messageId' },
    { name: 'templateId', align: 'left' as const, label: 'ID do Template', field: 'templateId' },
    { name: 'statusCode', align: 'left' as const, label: 'Status', field: 'statusCode' },
  ];
  ...
  const fetchApi = async (props: any) => {
    ...
    try {
      ...
      const response = await apiClientes.get(
        `/clientes-api/v1/emails-sent?page=${pagination.value.page}&size=${pagination.value.rowsPerPage}`,
        {
          headers: {
